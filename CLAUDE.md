...
## Project conventions (aws-solution-architect-associate-resources Bangla study site)
...
- Internal links inside `.astro` components must go through `siteUrl()` from `src/lib/urls.ts` (site is served under `/aws-solution-architect-associate-resources`).
- The same applies to `.mdx` content: Astro adds no `base` to links written in Markdown or MDX, so `/learn/` 404s in production. Write `<a href={siteUrl('learn/')}>…</a>` instead. Starlight hero-action links in frontmatter cannot call `siteUrl()` and must spell out the base (`/aws-solution-architect-associate-resources/learn/`). `pnpm check:links` fails on any root-relative link missing the base.
